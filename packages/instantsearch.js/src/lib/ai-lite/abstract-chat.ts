...
export abstract class AbstractChat<TUIMessage extends UIMessage> {
  ...
  get phase(): ChatPhase {
    ...
    const lastPart = this.lastProgressPart;
    ...
    // A step boundary, a file, a source: the turn is producing, but the part
  ...
  /**
   * The last part that says something about the turn's progress, skipping the
   * ones that render nothing. This — not `lastPart` — is what "what is this
   * turn doing" has to read, or an invisible trailing part answers for it.
   */
  get lastProgressPart(): TUIMessage['parts'][number] | undefined {
    const parts = this.lastMessage?.parts;
