...
`);
  ...
  describe('searchForFacetValues with composition', () => {
    it('uses the correct helper state for SFFV in nested index with composition', async () => {
        ...
        searchForFacetValues: jest.fn(() =>
          Promise.resolve({
            results: [
              {
                facetHits: [
                  { value: 'Apple', highlighted: 'Apple', count: 100 },
                ],
                exhaustiveFacetsCount: true,
                processingTimeMS: 1,
              },
      ...
      // Create a refinementList widget to test SFFV
      const refinementList = connectRefinementList(() => {})({
        attribute: 'brand',
      });

      childIndex.addWidgets([refinementList]);
      search.addWidgets([childIndex]);

      search.start();

      await wait(0);

      // Get the child index helper and configure facets
      const childHelper = childIndex.getHelper();
      childHelper!.setQueryParameter('facets', ['brand']);

      // Add some state to the child helper
      childHelper!.setQueryParameter('query', 'test query');
      childHelper!.setQueryParameter('page', 5);

      // Call searchForFacetValues on the child helper
      await childHelper!.searchForFacetValues('brand', 'Apple', 10, {
        highlightPreTag: '<mark>',
        highlightPostTag: '</mark>',
      });

      // Verify that searchForFacetValues was called on the mainHelper with the correct merged state
      expect(searchClient.searchForFacetValues).toHaveBeenCalledTimes(1);
      expect(searchClient.searchForFacetValues).toHaveBeenCalledWith({
        compositionID: 'childIndex',
        facetName: 'brand',
    ...
    it('uses the correct helper state for SFFV in non-composition nested index', async () => {
      const searchClient = createSearchClient({
        searchForFacetValues: jest.fn(() =>
          Promise.resolve([
            {
              facetHits: [
                { value: 'Samsung', highlighted: 'Samsung', count: 50 },
              ],
              exhaustiveFacetsCount: true,
              processingTimeMS: 1,
            },
      ...
      // Add some state to the child helper
      childHelper!.setQueryParameter('query', 'another query');
      childHelper!.setQueryParameter('hitsPerPage', 20);

      // Call searchForFacetValues on the child helper
      await childHelper!.searchForFacetValues('brand', 'Samsung', 15, {
        highlightPreTag: '<em>',
        highlightPostTag: '</em>',
      });

      // Verify that searchForFacetValues was called with the correct state
      expect(searchClient.searchForFacetValues).toHaveBeenCalledTimes(1);
      expect(searchClient.searchForFacetValues).toHaveBeenCalledWith(
        expect.arrayContaining([
          {
