...
export const index = (widgetParams: IndexWidgetParams): IndexWidget => {
  if (
  ...
  let localInstantSearchInstance: InstantSearch<UiState, UiState> | null = null;
  ...
  return {
    ...
    removeWidgets(widgets) {
      ...
      if (localInstantSearchInstance && Boolean(flatWidgets.length)) {
        ...
        helper!.recommendState = getLocalWidgetsRecommendParameters(
          localWidgets,
          {
            uiState: localUiState,
            initialRecommendParameters:
    ...
    init({ instantSearchInstance, parent, uiState }: IndexInitOptions) {
      ...
      // step.
      const mainHelper = instantSearchInstance.helper!;
      ...
      derivedHelper.on('result', ({ results }) => {
        ...
        // search behavior.
        helper!.lastResults = results;
        lastValidSearchParameters = results?._state || null;
      });
