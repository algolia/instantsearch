...
import { instantsearch, connectSearchBox } from '../..';
...
describe('router', () => {
  ...
  it('warns if initialUiState and routing are used together', () => {
    ...
    expect(() => {
      ...
      '[InstantSearch]: Using `initialUiState` together with routing is not recommended. The `initialUiState` will be overwritten by the URL parameters.'
    );
  });
