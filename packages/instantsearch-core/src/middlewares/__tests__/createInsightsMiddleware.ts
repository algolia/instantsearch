...
describe('insights', () => {
  ...
  describe('initialize', () => {
    ...
    it('warns when search client does not have credentials', () => {
      ...
      expect(() => {
        ...
        '[InstantSearch]: could not extract Algolia credentials from searchClient in insights middleware.'
      );
    });
  ...
  test('does not immediately set userToken if a rerender is expected', async () => {
    ...
    instantSearchInstance.addWidgets([
      connectDynamicWidgets(() => {})({
        widgets: [connectRefinementList(() => {})({ attribute: 'brand' })],
      }),
      connectHits(() => {})({}),
    ]);
    ...
    // Dynamic widgets will trigger 2 searches. We need to ensure the userToken is always set before the first search.

    const userToken = getUserToken();
    expect(userToken).toMatch(/^anonymous-/);
