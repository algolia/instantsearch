...
function exportCSSVariables(values: CSSVariables): string {
...
export function createInstantSearchTweakpane(
  ...
  // Dynamically import Pane to avoid bundling it if not used
  ...
  spacingFolder
    ...
    .on('change', (ev: any) => {
      updateCSSVariable('--ais-spacing', `${ev.value / 16}rem`);
    });
  ...
  zIndexFolder
    ...
    .on('change', (ev: any) =>
      updateCSSVariable('--ais-z-index-chat', ev.value)
    );
  ...
  exportFolder.addButton({ title: 'Copy to clipboard' }).on('click', () => {
    const cssVariables = exportCSSVariables(values);
    navigator.clipboard
