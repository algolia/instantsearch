...
import {
  ...
  type StickToBottomCoreController,
  type StickToBottomCoreRequest,
  type StickToBottomCoreState,
    ...
    >(() => {
      ...
      const createRequest = (
        ...
        const request: StickToBottomCoreRequest = {
          ...
          preserveScrollPosition: scrollOptions.preserveScrollPosition ?? false,
        };
        ...
        if (scrollOptions.duration instanceof Promise) {
          ...
          request.durationElapsed = waitElapsed + (scrollOptions.duration ?? 0);
        }
      ...
      return {
        ...
        createResizeRequest(initial) {
          const behavior = mergeAnimations(
            optionsRef.current,
            initial ? optionsRef.current.initial : optionsRef.current.resize
          );
          ...
          return createRequest({
            ...
            duration:
              behavior === 'instant' ? undefined : RETAIN_ANIMATION_DURATION_MS,
          });
        },
      ...
      // Keep the controller stable while callbacks read current options through optionsRef.
