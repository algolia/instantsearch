...
describe('Chat', () => {
  ...
  test('updates open persistence when the option changes', async () => {
    ...
    function Subject({ persistOpen }: { persistOpen: boolean }) {
      return (
        <InstantSearch searchClient={searchClient} indexName="indexName">
          <Chat
            ref={chatRef}
    ...
    const { container, rerender } = render(<Subject persistOpen={false} />);
    ...
    expect(screen.getByText('I found one option.')).toBeInTheDocument();

    externalButton.focus();
    rerender(<Subject persistOpen={true} />);
    await act(async () => {
      await wait(0);
    });
    ...
    expect(sessionStorage.getItem(cacheKey)).toBe('true');

    externalButton.focus();
    rerender(<Subject persistOpen={false} />);
    await act(async () => {
      await wait(0);
    });
