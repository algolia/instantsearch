...
export const useStickToBottom = (
  ...
  const setIsAtBottom = useCallback((newIsAtBottom: boolean) => {
    ...
    updateIsAtBottom(newIsAtBottom);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const setEscapedFromLock = useCallback((escaped: boolean) => {
    state.escapedFromLock = escaped;
    updateEscapedFromLock(escaped);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
...
function useRefCallback<T extends (ref: HTMLElement | null) => any>(
  ...
  const result = useCallback((ref: HTMLElement | null) => {
    ...
    return callback(ref);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, deps) as any as React.RefObject<HTMLElement | null> &
