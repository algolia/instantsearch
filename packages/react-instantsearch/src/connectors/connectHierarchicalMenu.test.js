...
describe('connectHierarchicalMenu', () => {
  describe('single index', () => {
    ...
    it('items value function should clear it from the search state', () => {
      ...
      expect(searchState).toEqual({page: 1, hierarchicalMenu: {one: '', two: 'two'}});
    });
    ...
    it('should return the right searchState when clean up', () => {
      ...
      expect(searchState).toEqual({another: {searchState: 'searchState'}, hierarchicalMenu: {}});
    });
  });
  describe('multi index', () => {
    ...
    it('calling refine updates the widget\'s search state', () => {
      ...
      let nextState = refine(
        {attributes: ['ok']},
        {indices: {first: {otherKey: 'val', hierarchicalMenu: {otherKey: 'val'}}}},
       'yep'
       );
      ...
      nextState = refine(
        {attributes: ['ok']},
        {indices: {first: {otherKey: 'val', hierarchicalMenu: {ok: 'yep', otherKey: 'val'}}}},
        'yep'
      );

      expect(nextState).toEqual({indices: {
        first: {otherKey: 'val', hierarchicalMenu: {ok: 'yep', otherKey: 'val'}},
    ...
    it('correctly applies its state to search parameters', () => {
      ...
      params = getSP(initSP, {
        ...
        showParentLevel: true,
        limitMin: 1,
      }, {indices: {first: {otherKey: 'val', hierarchicalMenu: {ATTRIBUTE: 'ok'}}}});
      expect(params).toEqual(
        initSP
          .addHierarchicalFacet({
            name: 'ATTRIBUTE',
    ...
    it('items value function should clear it from the search state', () => {
      ...
      const searchState = metadata.items[0].value({indices: {first: {hierarchicalMenu: {one: 'one', two: 'two'}}}});

      expect(searchState).toEqual({indices: {first: {page: 1, hierarchicalMenu: {one: '', two: 'two'}}}});
    });

    it('should return the right searchState when clean up', () => {
      let searchState = cleanUp(
        {attributes: ['one']},
        {indices: {first: {hierarchicalMenu: {one: 'one', two: 'two'}, another: {searchState: 'searchState'}}}}
      );
      expect(searchState).toEqual({indices: {
        first: {hierarchicalMenu: {two: 'two'}, another: {searchState: 'searchState'}}},
      });

      searchState = cleanUp({attributes: ['two']}, searchState);
      expect(searchState).toEqual({indices: {first: {another: {searchState: 'searchState'}, hierarchicalMenu: {}}}});
    });
  });
});
