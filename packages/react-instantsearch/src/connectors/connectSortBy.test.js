...
describe('connectSortBy', () => {
  ...
  describe('multi index', () => {
    const context = {context: {ais: {mainTargettedIndex: 'first'}, multiIndexContext: {targettedIndex: 'first'}}};
    const getProvidedProps = connect.getProvidedProps.bind(context);
    const getSP = connect.getSearchParameters.bind(context);
    const cleanUp = connect.cleanUp.bind(context);

    it('provides the correct props to the component', () => {
      props = getProvidedProps({items: [{value: 'yep'}, {value: 'yop'}]}, {indices: {first: {sortBy: 'yep'}}});
      expect(props).toEqual({
        items: [{value: 'yep', isRefined: true}, {value: 'yop', isRefined: false}],
        currentRefinement: 'yep',
      });
    ...
    it('refines the index parameter', () => {
      params = getSP(new SearchParameters(), {}, {indices: {first: {sortBy: 'yep'}}});
      expect(params.index).toBe('yep');
    });

    it('should return the right searchState when clean up', () => {
      const searchState = cleanUp({}, {indices: {first: {sortBy: {searchState: 'searchState'}}},
        another: {searchState: 'searchState'}});
      expect(searchState).toEqual({indices: {first: {}}, another: {searchState: 'searchState'}});
    });
  });
});
