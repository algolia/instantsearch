...
describe('connectRange', () => {
  describe('single index', () => {
    ...
    it('should return the right searchState when clean up', () => {
      ...
      expect(searchState).toEqual({range: {}, another: {searchState: 'searchState'}});
    });
  });
  describe('multi index', () => {
    ...
    it('calling refine updates the widget\'s search state', () => {
      ...
      let nextState = refine({attributeName: 'ok'},
        {otherKey: 'val', indices: {first: {range: {otherKey: {min: 1, max: 2}}}}}, {min: 3, max: 5});
      expect(nextState).toEqual({
        otherKey: 'val',
    ...
    it('registers its filter in metadata', () => {
      ...
      const searchState = metadata.items[0].value({indices: {first: {range: {wot: {min: 5}}}}});
    ...
    it('items value function should clear it from the search state', () => {
      const metadata = getMetadata({attributeName: 'one'}, {indices: {first: {range: {one: {min: 5}, two: {max: 4}}}}});

      const searchState = metadata.items[0].value({indices: {first: {range: {one: {min: 5}, two: {max: 4}}}}});

      expect(searchState).toEqual({indices: {first: {range: {two: {max: 4}}}}});
    });

    it('should return the right searchState when clean up', () => {
      let searchState = cleanUp({attributeName: 'name'}, {
        indices: {first: {range: {name: 'searchState', name2: 'searchState'}}},
        another: {searchState: 'searchState'},
      });
      expect(searchState).toEqual({indices: {first: {range: {name2: 'searchState'}}},
        another: {searchState: 'searchState'}});
