...
describe('connectPagination', () => {
  ...
  describe('multi index', () => {
    ...
    it('provides the correct props to the component', () => {
      props = getProvidedProps({}, {}, {results: {first: {nbPages: 666}}});
      expect(props).toEqual({currentRefinement: 1, nbPages: 666, canRefine: true});

      props = getProvidedProps({}, {indices: {first: {page: 5}}}, {results: {first: {nbPages: 666}}});
      expect(props).toEqual({currentRefinement: 5, nbPages: 666, canRefine: true});
    });
    ...
    it('calling refine updates the widget\'s search state', () => {
      ...
      let nextState = refine({}, {indices: {first: {otherKey: 'val'}}}, 'yep');
      expect(nextState).toEqual({indices: {first: {
        otherKey: 'val',
        page: 'yep',
      }}});
    ...
    it('should return the right searchState when clean up', () => {
      const newState = cleanUp({}, {indices: {first: {
        page: {searchState: 'searchState'}, another: {searchState: 'searchState'}},
      }});
      expect(newState).toEqual({indices: {first: {another: {searchState: 'searchState'}}}});
    });
  });
});
