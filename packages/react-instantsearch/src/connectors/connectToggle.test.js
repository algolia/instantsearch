...
describe('connectToggle', () => {
  describe('single index', () => {
    ...
    it('items value function should clear it from the search state', () => {
      ...
      expect(searchState).toEqual({page: 1, toggle: {one: false, two: false}});
    });
    ...
    it('should return the right searchState when clean up', () => {
      ...
      expect(searchState).toEqual({toggle: {}, another: {searchState: 'searchState'}});
    });
  });
  describe('multi index', () => {
    ...
    it('provides the correct props to the component', () => {
      ...
      props = getProvidedProps({attributeName: 't'}, {indices: {first: {toggle: {t: true}}}});
      expect(props).toEqual({currentRefinement: true});
    });
    ...
    it('calling refine updates the widget\'s search state', () => {
      ...
      let searchState = refine(
        {attributeName: 't'},
        {otherKey: 'val', indices: {first: {toggle: {otherKey: false}}}},
        true,
      );
      expect(searchState).toEqual({
        otherKey: 'val',
      ...
      expect(searchState).toEqual({
        indices: {first: {toggle: {t: true, otherKey: false}}, second: {page: 1, toggle: {t: false}}},
      });
    });
    ...
    it('registers its filter in metadata', () => {
      const metadata = getMetadata({attributeName: 't', label: 'yep'}, {indices: {first: {toggle: {t: true}}}});
      expect(metadata).toEqual({
        items: [
          {
            label: 'yep',
    ...
    it('items value function should clear it from the search state', () => {
      ...
      const searchState = metadata.items[0].value({indices: {first: {toggle: {one: true, two: false}}}});

      expect(searchState).toEqual({indices: {first: {page: 1, toggle: {one: false, two: false}}}});
    });

    it('should return the right searchState when clean up', () => {
      let searchState = cleanUp({attributeName: 'name'}, {
        indices: {first: {toggle: {name: 'searchState', name2: 'searchState'}}},
        another: {searchState: 'searchState'},
      });
      expect(searchState).toEqual({
      ...
      searchState = cleanUp({attributeName: 'name2'}, searchState);
      expect(searchState).toEqual({indices: {first: {toggle: {}}}, another: {searchState: 'searchState'}});
    });
  });
});
