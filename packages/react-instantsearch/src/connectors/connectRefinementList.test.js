...
describe('connectRefinementList', () => {
  describe('single index', () => {
    ...
    it('provides the correct props to the component', () => {
      ...
      expect(props).toEqual({
        items: [], currentRefinement: [], isFromSearch: false,
        canRefine: false,
      });
    ...
    it('items value function should clear it from the search state', () => {
      ...
      expect(searchState).toEqual({page: 1, refinementList: {one: ['one2'], two: ['two']}});

      searchState = metadata.items[0].items[1].value(searchState);

      expect(searchState).toEqual({page: 1, refinementList: {one: '', two: ['two']}});
    });
    ...
    it('should return the right searchState when clean up', () => {
      ...
      expect(searchState).toEqual({refinementList: {}, another: {searchState: 'searchState'}});
    });
  ...
  describe('multi index', () => {
    ...
    it('provides the correct props to the component', () => {
      ...
      props = getProvidedProps({attributeName: 'ok'}, {indices: {first: {refinementList: {ok: ['wat']}}}}, {results});
      expect(props).toEqual({
        items: [], currentRefinement: ['wat'], isFromSearch: false,
        canRefine: false,
      });
    ...
    it('calling refine updates the widget\'s search state', () => {
      ...
      let nextState = refine({attributeName: 'ok'},
      {otherKey: 'val', indices: {first: {refinementList: {otherKey: ['val']}}}}, ['yep']);
      expect(nextState).toEqual({
        otherKey: 'val',
    ...
    it('items value function should clear it from the search state', () => {
      const metadata = getMetadata(
        {attributeName: 'one'},
        {indices: {first: {refinementList: {one: ['one1', 'one2'], two: ['two']}}}}
      );

      let searchState = metadata.items[0].items[0].value({indices:
        {first: {refinementList: {one: ['one1', 'one2'], two: ['two']}}},
      });

      expect(searchState).toEqual({indices: {first: {page: 1, refinementList: {one: ['one2'], two: ['two']}}}});

      searchState = metadata.items[0].items[1].value(searchState);

      expect(searchState).toEqual({indices: {first: {page: 1, refinementList: {one: '', two: ['two']}}}});
    });

    it('should return the right searchState when clean up', () => {
      let searchState = cleanUp({attributeName: 'name'}, {
        indices: {first: {refinementList: {name: 'searchState', name2: 'searchState'}}},
        another: {searchState: 'searchState'},
      });
      ...
      searchState = cleanUp({attributeName: 'name2'}, searchState);
      expect(searchState).toEqual({indices: {first: {refinementList: {}}}, another: {searchState: 'searchState'}});
    });
  });
});
