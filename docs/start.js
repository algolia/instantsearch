...
watch([
  rootPath('docs/assets/'),
  rootPath('docs/src/**/*'),
  rootPath('docs/layouts/**/*.pug'),
], {
  ignoreInitial: true,
  ...
  .on('all', (event, filePath) => {
    ...
    const isSrcFileChange = filePath.includes('src/') && !filePath.includes('docs')
    const isSassFile = filePath.includes('docs') && /^[^_.].*\.s[ac]ss/.test(filePath)
    const isMarkdownOrPugFile = filePath.includes('docs') && /\.md$|\.pug$/.test(filePath)
