...
const router = historyRouter<RouteState>({
  ...
  parseURL({ qsModule, location }): RouteState {
    ...
    const allLabels = Array.isArray(labels) ? labels : [labels].filter(Boolean);
    ...
    return {
      ...
      types: allTypes.map((type) => decodeURIComponent(type as string)),
      labels: allLabels.map((label) => decodeURIComponent(label as string)),
    };
  },
});
