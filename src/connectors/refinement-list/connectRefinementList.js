...
const connectRefinementList = refinementListRendering => ({
    container,
    attributeName,
    operator = 'or',
    sortBy = ['count:desc', 'name:asc'],
  ...
  const render = (facetValues, state, createURL,
                  helperSpecializedSearchFacetValues, templateProps,
                  toggleRefinement, isFromSearch, isFirstSearch) => {
    ...
    refinementListRendering({
      ...
      limitMin: limit,
      shouldAutoHideContainer: autoHideContainer && !isFromSearch && facetValues.length === 0,
      ...
      if (query === '' && lastResultsFromMainSearch) {
        ...
        helper.searchForFacetValues(attributeName, query).then(results => {
          ...
          render(
            ...
            helperSpecializedSearchFacetValues, templateProps, toggleRefinement, true, false);
        });
      }
    };
  ...
  return {
    ...
    init({templatesConfig, helper, createURL}) {
      ...
      render([], helper.state, createURL, this.searchFacetValues,
             this._templateProps, this.toggleRefinement, false, true);
    },
    render({results, state, createURL}) {
      ...
      render(facetValues, state, createURL, this.searchFacetValues,
             this._templateProps, this.toggleRefinement, false, false);
    },
  };
};
