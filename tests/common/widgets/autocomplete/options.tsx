...
export function createOptionsTests(
  ...
  describe('options', () => {
    ...
    describe('prompt suggestions', () => {
      ...
      test('shows fallback "Ask about" when no prompt suggestions match the query', async () => {
        const searchClient = createSearchClient({
          ...
          search: jest.fn((requests) =>
            Promise.resolve(
              createMultiSearchResponse(
                ...requests.map(
                  (req: { indexName: string; params?: { query?: string } }) =>
                    createSingleSearchResponse({
                      index: req.indexName,
                      hits:
                        // eslint-disable-next-line no-nested-ternary
                        req.indexName === 'prompt_suggestions' &&
                        req.params?.query
                          ? []
                          : req.indexName === 'prompt_suggestions'
                          ? [
                              {
                                objectID: '1',
                                prompt: 'What is the best product?',
                              },
                            ]
                          : [],
                    })
        ...
        // Initially shows prompt suggestions from the index
        expect(
          document.querySelectorAll('.ais-AutocompletePromptSuggestionsItem')
        ).toHaveLength(1);
        expect(
          document.querySelector('.ais-AutocompletePromptSuggestionsItem')
        ).toHaveTextContent('What is the best product?');

        // Type a query that returns no results from the prompt suggestions index
        const input = screen.getByRole('combobox', { name: /submit/i });
        await act(async () => {
          userEvent.click(input);
          await userEvent.paste(input, 'something unique');
          await wait(0);
        });

        // Should show fallback "Ask about" suggestion
        const fallbackItems = document.querySelectorAll(
          '.ais-AutocompletePromptSuggestionsItem'
        );
        expect(fallbackItems).toHaveLength(1);
        expect(fallbackItems[0]).toHaveTextContent(
          'Ask about "something unique"'
        );
      });
